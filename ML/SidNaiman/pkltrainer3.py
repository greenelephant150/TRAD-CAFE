#!/usr/bin/env python3
"""
🎯 SID METHOD AI MODEL TRAINER - FULL HISTORICAL DATA
========================================================
...
try:
    ...
    print_success("XGBoost available (CPU)")
except ImportError:
    XGBOOST_AVAILABLE = False
    print_warning("XGBoost not available")
...
def train_random_forest_gpu(X_train, y_train, X_val, y_val, verbose=True):
    ...
    try:
        ...
        if verbose:
            print_success(f"GPU Random Forest accuracy: {accuracy:.4f}")
...
def train_random_forest_cpu(X_train, y_train, X_val, y_val, verbose=True):
    ...
    if verbose:
        print_success(f"CPU Random Forest accuracy: {accuracy:.4f}")
...
def train_xgboost_cpu(X_train, y_train, X_val, y_val, verbose=True):
    ...
    if verbose:
        print_success(f"CPU XGBoost accuracy: {accuracy:.4f}")
...
def create_training_samples(df: pd.DataFrame, lookback: int = 10, 
    ...
    """Create training samples with progress bar"""
    ...
    iterator = range(lookback, len(df) - 1, step)
...
def train_pair(pair: str, target: str = 'target_direction', 
    ...
    if use_gpu and CUML_AVAILABLE:
        rf_model, rf_acc = train_random_forest_gpu(X_train, y_train, X_val, y_val, verbose)
        if rf_model:
            models['random_forest_gpu'] = rf_model
            accuracies['random_forest_gpu'] = rf_acc
    else:
        rf_model, rf_acc = train_random_forest_cpu(X_train, y_train, X_val, y_val, verbose)
        if rf_model:
    ...
    xgb_model, xgb_acc = train_xgboost_cpu(X_train, y_train, X_val, y_val, verbose)
    if xgb_model:
...
def train_all_pairs(pairs: List[str], target: str = 'target_direction',
    ...
    if use_gpu:
        print_gpu(f"GPU acceleration: {'enabled' if CUML_AVAILABLE else 'not available'}")
    
    # Get date range info
    print_info("Determining overall date range...")
...
# ============================================================================
def main():
    parser = argparse.ArgumentParser(description='🎯 Sid Method AI Model Trainer (Full Historical Data)')
    ...
    print_header("🎯 SID METHOD AI MODEL TRAINER (FULL HISTORICAL DATA)")
    ...
    if args.pair:
        ...
        # TRAIN ALL 87 PAIRS
        print_info(f"🚀 Training ALL {len(all_pairs)} pairs (this will take time)")
        print_info(f"   Expected time: ~{len(all_pairs) * 5} minutes (approx)")
