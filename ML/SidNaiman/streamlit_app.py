...
Version: 5.0 - SID METHOD INTEGRATION
"""
...
try:
    ...
    from sid_method import SidMethod  # Import Sid Method
...
MODEL_BASE_PATH = "/mnt2/Trading-Cafe/ML/SNaiman2/ai/trained_models/"
...
def detect_sid_signals(df: pd.DataFrame, sid: SidMethod) -> List[Dict]:
    ...
    for i in range(max(0, len(df) - 20), len(df) - 1):
        ...
        # Check for oversold signal (RSI < 30)
        if current_rsi < 30:
            # Check MACD alignment (pointing up)
            if df['macd_aligned_up'].iloc[i]:
                # Check if MACD crossed (stronger confirmation)
        ...
        # Check for overbought signal (RSI > 70)
        elif current_rsi > 70:
            # Check MACD alignment (pointing down)
...
def display_train_models_tab():
    ...
    with col1:
        ...
        lookback = st.slider("Lookback Period", 5, 30, 10, 
                            help="Number of previous bars to use for features (Sid uses 10-20)")
        ...
        st.markdown("##### Features to Include (SID Method)")
        
        feature_categories = {
            'RSI Features': ['rsi', 'rsi_oversold', 'rsi_overbought', 'rsi_change'],
            'MACD Features': ['macd', 'macd_signal', 'macd_hist', 'macd_cross'],
            'Price Action': ['returns', 'body_ratio', 'wick_ratio'],
...
def display_model_performance_tab():
    ...
    st.markdown("#### 📈 Model Performance Metrics (SID Method)")
    ...
    if 'Accuracy' in models_df.columns and 'Pair' in models_df.columns:
        ...
        if not plot_df.empty:
            ...
            fig = px.bar(
                ...
                title="Model Accuracy by Pair (SID Method)",
...
def display_update_model_tab():
    ...
    with col2:
        ...
        new_lookback = st.slider("Lookback Period", 5, 30, 10)
...
def detect_sid_signals_with_ai(data: Dict[str, pd.DataFrame]) -> List[Dict]:
        ...
        for signal in signals:
            ...
            if (st.session_state.ai_enabled and 
                ...
                try:
                    ...
                    features = {}
                    idx = signal['index'] if 'index' in signal else -1
                    ...
                    signal['ai_model_used'] = ai_pred.get('model_used', None)
                    
                    # Adjust confidence with AI
            ...
            signal['combined_score'] = signal['confidence']
...
def display_signals(signals: List[Dict]):
    ...
    for signal in filtered:
        ...
        with st.container():
            ...
            with col3:
                badge_class = f"ai-{strength.lower().replace('_', '-')}"
                ...
                if signal.get('ai_confidence', 0) > 0:
                    ...
                    st.markdown(f"<span class='{badge_class}'>AI: {strength}</span>", unsafe_allow_html=True)
...
def render_trade_confirmation(signal: Dict):
    ...
    with col2:
        ...
        if signal_type == 'BUY':
            stop_loss = signal['price'] * 0.99  # 1% below entry
            take_profit = signal['price'] * 1.02  # 2% target
        else:
            stop_loss = signal['price'] * 1.01  # 1% above entry
            take_profit = signal['price'] * 0.98  # 2% target
        
        # Calculate position size (Sid's 1% rule)
    ...
    with col1:
        if st.button("✅ CONFIRM", type="primary", use_container_width=False):
            try:
                result = st.session_state.oanda_trader.place_order(
        ...
        if time_since > 60 or not st.session_state.current_data:
            with st.spinner("🔍 Scanning markets..."):
                ...
                if st.session_state.current_data:
                    st.session_state.detected_signals = detect_sid_signals_with_ai(st.session_state.current_data)
...
st.markdown(
    ...
    <div style='text-align: center; color: gray; padding: 1rem;'>
        Sid Naiman SID Method Trading System | v5.0 - SID METHOD INTEGRATION<br>
        ...
        Risk: 0.5-2% Per Trade | CSV Path: {CSV_BASE_PATH} | Parquet Path: {PARQUET_BASE_PATH}<br>
