...
class ModelManager:
    ...
    def discover_models(self) -> Dict[str, Dict]:
        ...
        for model_path in model_files:
            try:
                ...
                # Parse filename format for Sid Method
                # Format: DDMMYYYY--SidMethod--PAIR--TARGET--L{lookback}.pkl
                ...
                if len(parts) >= 5:
                    ...
                    pair = parts[2]
                    target = parts[3]
                    lookback_part = parts[4]
                    
                    # Convert date
                    min_date = datetime.strptime(date_str, "%d%m%Y")
                    max_date = min_date  # Sid models use single date
                    
                    # Extract lookback value
                    ...
                    discovered[filename] = {
                        ...
                        'model_name': f"{pair}_{max_date.strftime('%Y%m%d')}_{target}",
                        'method': 'SidMethod'
                    }
                    logger.debug(f"Added Sid Method model: {pair}")
                elif len(parts) >= 4:
                    # Try Simon Pullen format: DDMMYYYY--DDMMYYYY--AUTHOR--PAIR--TIMEFRAME.pkl
                    min_date_str, max_date_str, creator, pair = parts[:4]
                    ...
                    try:
                        ...
                        if model_data and isinstance(model_data, dict) and 'metadata' in model_data:
                            ...
                            discovered[filename] = {
                                ...
                                'target': meta.get('target', 'target_direction'),
    ...
    def load_model(self, model_filename: str) -> Optional[Dict]:
        ...
        Args:
            model_filename: Filename of the model (e.g., 'DDMMYYYY--SidMethod--PAIR--TARGET--L10.pkl')
        ...
        try:
            ...
            if hasattr(model, 'estimators_'):
                ...
                for i, estimator in enumerate(model.estimators_):
                    # Skip if estimator is numpy array (for Sid Method XGBoost models)
